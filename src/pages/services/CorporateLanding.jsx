...
import { ArrowRight, ScanLine, FileText, CheckCircle2, Building2 } from 'lucide-react';
...
const CorporateLanding = () => {
  ...
  const features = [
    { key: 'remote', icon: ScanLine, color: 'text-blue-400' },
    { key: 'billing', icon: FileText, color: 'text-qeyafa-gold' },
    { key: 'consistency', icon: CheckCircle2, color: 'text-green-400' },
  ];

  return (
    <div className="min-h-screen bg-qeyafa-black bg-grid-pattern selection:bg-qeyafa-gold/30">
      <SEO
        titleKey="seo.corporate.title"
        descriptionKey="seo.corporate.description"
        path="/services/b2b"
      />
      ...
      <section className="relative pt-32 pb-20 px-4 overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto text-center">
          ...
          >
            ...
            >
              <span>{t('corporate_landing.hero.cta')}</span>
              ...
              <ScrollReveal key={item.key} delay={index * 0.1}>
                <GlassCard className="h-full text-center p-8 hover:border-qeyafa-gold/50 transition-colors duration-300">
                  ...
                  <h3 className="text-xl font-bold text-white mb-3">
                    {t(`corporate_landing.features.${item.key}.title`)}
                  </h3>
                  <p className="text-white/60 leading-relaxed">
                    {t(`corporate_landing.features.${item.key}.desc`)}
                  </p>
                </GlassCard>
              </ScrollReveal>
            ))}
